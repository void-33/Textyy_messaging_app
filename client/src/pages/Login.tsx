...
const Login: React.FC = () => {
  ...
  const emailUsernameLabel = useRef<HTMLLabelElement>(null);
  ...
  useEffect(() => {
    updateLabel(emailUsernameLabel, creds.emailUsername);
  }, [creds.emailUsername]);
  useEffect(() => {
    updateLabel(passwordLabel, creds.password);
  }, [creds.password]);
  ...
  const handleSubmit = async (e: React.FormEvent): Promise<void> => {
    ...
    const body = {
      email: creds.emailUsername.includes('@') ? creds.emailUsername : undefined,
      username: !creds.emailUsername.includes('@') ? creds.emailUsername : undefined,
    ...
    try {
      ...
      if (response.data.success) {
        ...
        setCreds({ emailUsername: "", password: "" });
        ...
        <div className="flex flex-rows flex-wrap gap-20 items-center mx-20 p-4 justify-center min-w-[244px] ">
          ...
          <main className="flex-grow flex flex-col items-start justify-start bg-slate-50 rounded-md p-4 shadow-lg shadow-gray-600">
            <form
              ...
              className="w-full text-blue-800 text-xl"
            >
              {/* email field */}
              <div className="rounded-md h-11 mb-3 border-2 focus-within:border-blue-600 group relative w-full">
                <label
                  htmlFor="emailArea"
                  ref={emailUsernameLabel}
                  className="text-blue-500 rounded-md top-2 left-1 transition-all ease-in absolute group-focus-within:-translate-y-5 group-focus-within:scale-75 group-focus-within:z-50 group-focus-within:bg-slate-100 group-focus-within:hover:cursor-default hover:cursor-text"
                >
                  Email or Username
                </label>
                <input
                  type="text"
                  value={creds.emailUsername}
                  name="emailUsername"
                  id="emailArea"
                  onChange={handleInputChange}
                  required
                  className="bg-slate-100 rounded-md focus:outline-none w-full h-full px-2 text-lg"
                />
              </div>
              ...
              <div>
                <button
                  type="submit"
                  className="h-11 w-full mb-3 bg-blue-600 rounded-md text-white text-xl font-semibold"
                >
                  {" "}
                  Log In
                </button>
              ...
              <Link to="/" className="inline-block w-full text-center text-sm">
                Forgot password?
              </Link>
